...
def eval_genomes(genomes, config):
    ...
    if(count==0):
        ...
        if f.mode=='r':
            highest_points = f.read()
        count+=1
    
    while run:
        #display high-score on screen    
        text=font.render(f'Highest Points: {str(highest_points)}',True, (0,0,0))
        screen.blit(text,(750, 50)) #blit()=copy the contents of one Surface onto another Surface 
        ...
        if(flag_high_score==True):
            #input text
            highest_points = points  #score_collision = points
